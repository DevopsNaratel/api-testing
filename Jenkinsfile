pipeline {
    ...
    environment {
        // --- KONFIGURASI DOCKER ---
        DOCKER_IMAGE = 'diwamln/api-testing' 
        ...
        MANIFEST_TEST_PATH = 'api-testing/dev/deployment.yaml'
        MANIFEST_PROD_PATH = 'api-testing/prod/deployment.yaml' 
    }
                        ...
                        withCredentials([usernamePassword(credentialsId: GIT_CREDS, usernameVariable: 'GIT_USER', passwordVariable: 'GIT_PASS')]) {
                            ...
                            sh """
                                ...
                                if ! git diff-index --quiet HEAD; then
                                    git commit -m 'Deploy api-testing: ${env.BASE_TAG} [skip ci]'
                    ...
                    dir('temp_manifests') {
                        withCredentials([usernamePassword(credentialsId: GIT_CREDS, usernameVariable: 'GIT_USER', passwordVariable: 'GIT_PASS')]) {
                            ...
                            sh """
                                git add .
                                if ! git diff-index --quiet HEAD; then
                                    git commit -m 'Promote api-testing to PROD: ${env.BASE_TAG} [skip ci]'
                                    git push origin main
                                else
                                    echo "No changes detected."
                                fi
